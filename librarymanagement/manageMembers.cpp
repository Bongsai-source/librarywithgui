    ...
    , ui(new Ui::manageMembers)
{
    ...
    connect(ui->addMembersButton, &QPushButton::clicked, this, &manageMembers::on_addMembersButton_clicked);
    connect(ui->deleteMembersButton, &QPushButton::clicked, this, &manageMembers::on_deleteMembersButton_clicked);
}
...
void manageMembers::on_addMembersButton_clicked()
{
    ...
    QString genre = QInputDialog::getText(this, "Enter Book Genre", "Genre:", QLineEdit::Normal, QString(), &ok);
    if (!ok) {
        // User pressed "Cancel"
        return;
    }
...
void manageMembers::on_deleteMembersButton_clicked()
{
    bool ok; // Used to check if the user pressed "OK" or "Cancel"
...
void manageMembers::on_itemChanged(QTableWidgetItem *item)
{
    if (item) {
        // Optionally, you can add code here to handle when an item is changed
        // For example, you could display a message or update some internal data
